...
<div class="subnavbar">
 ...
 <?php
           $id = $_GET['i'];
            ...
            $sql = mysqli_query($conn, "SELECT * FROM ocupacion WHERE idOcupacion='$id'");
            if(mysqli_num_rows($sql) == 0){
                header('location: ../ocupaciones/');
            }else{
  ...
  <div class="col-2">
     <input  type="hidden" name="id" id="id" value="<?php echo $row['idOcupacion']; ?>" placeholder="Tidak perlu di isi" class="form-control span8 tip" readonly="readonly">
    <label>
      Tipo Documento
      <input name="nombre" id="nombre" value="<?php echo $row['nombre']?>" placeholder="Ingrese el nombre de la ocupación" required minlength="2" type="text">
    </label>
  </div>
  ...
  <div class="col-submit">
    <a href="../ocupaciones/" class="btn btn-danger"><i class="icon-remove"></i> Cancelar</a>
  </div>
