...
if(isset($_POST['update'])){
       ...
       $municipio  = mysqli_real_escape_string($conn,(strip_tags($_POST['municipio'], ENT_QUOTES)));
        ...
        $aboutme  = mysqli_real_escape_string($conn,(strip_tags($_POST['aboutme'], ENT_QUOTES)));
				...
				$update = mysqli_query($conn, "UPDATE chefs SET nombres='$nombres', apellidos='$apellidos', idTipoDocumento='$tipodocumento', n_Documento='$documento',
         direccion='$direccion', idMunicipio='$municipio', codigopostal='$codigopostal', aboutme='$aboutme'  WHERE idUsuario='$id'") or die(mysqli_error());
				if($update){
					echo "<script>
                              Swal.fire({
                                icon: 'success',
